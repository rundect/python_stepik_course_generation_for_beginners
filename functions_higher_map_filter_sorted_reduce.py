...
filter_result = list(filter(lambda y: y[2] == 'primary' and y[1] > 10000000, data))
map_result = list(map(lambda x: x[0], filter_result))
sorted_result = sorted(map_result, key=lambda x: x)
print('Cities: ' + reduce(lambda s1, s2: s1 + ', ' + s2, sorted_result))